...
public class BonusModeHand extends Hand {
		...
		if (getScoreCard().getHasBonus()) {
			...
			if ((countCards[0] == 5 || countCards[1] == 5 || countCards[2] == 5
				...
				if (counter < 7) {
					...
					getScoreCard().getHandFromHands(counter).setScore(counter * 5);
				} else {
					// award bonus because top section part for this set is
					// already complete
					getScoreCard().setDicezyBonus(100);
					if (getScoreCard().getHandFromHands(7).getScore() == -1
							|| getScoreCard().getHandFromHands(8).getScore() == -1
							|| getScoreCard().getHandFromHands(9).getScore() == -1
							|| getScoreCard().getHandFromHands(10).getScore() == -1
							|| getScoreCard().getHandFromHands(11).getScore() == -1
							|| getScoreCard().getHandFromHands(12).getScore() == -1) {
						scoreJoker();
					} else {
						scoreForfeit();
					}
	...
	private void scoreJoker() {
		...
		while (true) {
			...
			if (i >= 7 && i <= 13) {
				joker = (NamedHand) getScoreCard().getHandFromHands(i);
	...
	private void scoreForfeit() {
		...
		while (true) {
			int i = ConsoleInputter.getHex();
			if (i >= 1 && i <= 6) {
				joker = getScoreCard().getHandFromHands(i);
				if (joker.getScore() == -1) {
					joker.setScore(0);
					break;
				}
